...
- 작업 시작 시 모든 문서를 읽지 않는다. 먼저 `docs/ai/AI_INDEX.md`에서 작업 유형을 고르고 필요한 문서만 읽는다.
- 사용자가 "관련 MD 참고"라고 말하면 대화 기억이 아니라 `docs/ai/AI_INDEX.md`의 문서 선택 기준을 따른다.
...
- 에이전트는 요청받은 경우 파일 검사, 빌드, 문법 검사는 할 수 있지만 실행 중인 서버 프로세스를 임의로 조작하지 않는다.
- 서버 실행 확인이 필요하면 서버를 켜거나 끄기 전에 먼저 사용자에게 확인한다.
