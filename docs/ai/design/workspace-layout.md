...
`입출고 관리`는 관리 영역의 접힘 메뉴로 둔다. 기본 메뉴와 같은 글자 크기와 색상 톤을 쓰고, 펼쳐진 하위 메뉴인 `입고 관리`, `출고 관리`는 한 단계 낮은 작은 글자 크기로 표시한다. 하위 메뉴의 현재 화면 표시는 진한 배경 채움보다 얇은 강조선과 텍스트 색상으로 낮게 표현한다.
