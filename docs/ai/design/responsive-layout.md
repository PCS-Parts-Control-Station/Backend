...
4. 좌측 사이드바는 기본 닫힘 오프캔버스로 두어 본문 가용 폭을 확보한다.
...
- 사이드바는 항상 본문 밖의 오프캔버스로 열어 본문 폭을 줄이지 않는다.
- 등록/처리 화면의 오른쪽 업무 흐름 보조 패널은 너무 일찍 아래로 내리지 않는다.
...
- 사이드바는 화면 폭과 관계없이 `position: fixed`, `height: 100dvh`인 오프캔버스로 사용한다.
- 초기 상태는 닫힘이며 본문은 항상 전체 폭을 사용한다.
- 햄버거 버튼으로 열고, 배경 클릭 또는 `Escape` 키로 닫는다.
...
모든 화면에서 동일한 기본 닫힘 오프캔버스 구조를 사용한다.
...
조회 중심 관리 화면에서 전표, 관리번호, 품목, 분류, 거래처, 사용자, 검수 템플릿 행을 선택하면 오른쪽 상세 슬라이드 패널을 연다. 등록이 필요한 관리 화면은 헤더의 등록 버튼으로 같은 패널의 등록 모드도 열 수 있다.
...
- `width: min(520px, calc(100% - 32px))`처럼 화면보다 커지지 않게 한다.
- 좁은 화면에서는 `width: 100vw`를 쓰지 않고 `left/right` inset으로 폭을 잡아 브라우저 스크롤바나 viewport 계산 때문에 오른쪽 끝이 잘리지 않게 한다.
- `position: fixed`로 viewport 오른쪽에 붙인다.
- 화면 높이를 최대한 사용하되, 드로어 자체와 드로어 패널 자체는 세로 스크롤 컨테이너가 되면 안 된다.
- 제목, 닫기 버튼, 주요 액션 영역은 고정하고, 긴 본문은 `drawer-scroll-body` 같은 내부 본문 컨테이너만 세로 스크롤한다.
- 주요 액션 영역은 `form-actions`를 사용하며, 본문 내용이 길어져도 드로어 하단에 남아 있어야 한다.
- 라인 목록, 사양 항목, 타임라인처럼 반복 항목이 길어지는 경우에는 `drawer-scroll-body` 안의 `right-side-scroll-list` 같은 내부 목록 컨테이너에만 스크롤을 둔다.
- 배경 오버레이를 깔지 않는다.
- 패널이 열려도 본문 스크롤을 잠그지 않는다.
- 관리형 작업 드로어는 본문 외부 클릭으로 닫고, 다른 목록 행 클릭 시에는 열린 상태로 상세 내용만 교체한다.
- 전표형 상세 패널의 외부 클릭 동작은 `workflow-panel.md`를 따른다.
...
- 사이드바가 기본 닫힘이므로 본문 폭이 충분하면 한 줄 폼을 유지할 수 있다.
...
- 매우 좁은 폭에서 카드형으로 전환할 때도 전표 번호, 상태, 입고 내용, 수량, 입고일, 관리 버튼의 우선순위를 유지한다.
