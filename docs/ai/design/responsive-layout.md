...
```text
1520px 초과: 데스크톱 고정 사이드바, 햄버거 버튼으로 접기/펼치기
...
`1520px` 초과에서는 기존 업무 레이아웃을 유지하되, 햄버거 버튼으로 좌측 사이드바를 접거나 펼칠 수 있다.
...
```text
1600px: 데스크톱 사이드바, 햄버거 접기/펼치기, 2컬럼 겹침 없음
...
- 1520px 초과에서는 햄버거 클릭 시 사이드바가 접히거나 펼쳐진다.
- 1520px 이하에서는 햄버거 클릭 시 메뉴가 왼쪽에서 열린다.
- 배경은 흐려지고 클릭하면 닫힌다.
- `Escape` 키로 닫힌다.
