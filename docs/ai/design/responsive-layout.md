...
업무 화면은 검색 카드, 목록 테이블, 오른쪽 보조 패널을 동시에 쓰는 경우가 많다.

반응형 기준의 우선순위는 아래 순서다.

1. 본문 영역과 오른쪽 패널이 겹치지 않아야 한다.
2. 중간 폭에서는 좌측 사이드바를 접어 본문 가용 폭을 확보한다.
3. 작은 화면에서는 테이블을 억지로 줄이지 않고 카드형 또는 가로 스크롤 패턴으로 전환한다.
4. 햄버거 메뉴는 열림/닫힘 상태, 배경 오버레이, 키보드 닫기를 제공한다.
...
- 오른쪽 보조 패널을 너무 일찍 아래로 내리기보다, 중간 폭에서는 사이드바를 접어 본문 폭을 확보한다.
- 그래도 본문과 오른쪽 패널을 함께 두기 어렵다면 `1180px` 이하에서 1컬럼으로 전환한다.

## 데스크톱 레이아웃

`1520px` 초과에서는 기존 업무 레이아웃을 유지한다.
...
- 사이드바는 `position: sticky`, `height: 100vh`를 유지한다.
- 햄버거 버튼은 보이지 않는다.
- `content-grid`는 `minmax(0, 1fr) 336px` 구조를 기본으로 한다.
- 오른쪽 `side-panel`은 336px 전후로 둔다.

## 중간 폭 사이드바

`1520px` 이하에서는 좌측 사이드바를 오프캔버스로 전환한다.
...
- 페이지 아이콘과 제목은 `page-title-row` 안에서 유지한다.
...
```text
1520px 초과: 사이드바 고정 + 본문 2컬럼
1520px 이하: 사이드바 접힘 + 본문 2컬럼 유지 가능
1180px 이하: 본문 1컬럼
```
...
- 오른쪽 패널은 본문을 가리지 않아야 한다.
- `1180px` 이하에서는 오른쪽 패널을 목록 아래로 내린다.
...
- 모바일에서 입력창이 화면 밖으로 밀리면 개별 화면 예외보다 공통 `.stock-filter-form` 전환 기준을 먼저 조정한다.
- 거래처 화면처럼 필터가 4개 이상인 경우에도 검색어 입력칸은 우선 넓게 두고, 선택 필터와 검색 버튼은 중간 폭에서 줄어들 수 있는 고정 범위로 둔다.
- 거래처 화면은 한 줄 유지가 어려워지는 `1280px` 전후에서 `검색어`를 한 줄 전체로 올리고, `유형 / 역할 / 거래 상태 / 검색`을 다음 줄에 둔다.
- 거래처 화면은 태블릿 폭에서 위 2줄 검색 폼을 유지할 수 있고, 더 좁은 모바일 폭에서만 전체 1열로 내린다.
- 거래처 목록 요약은 데스크톱과 태블릿 폭에서 목록 헤더 오른쪽 끝에 붙이고, 모바일 폭에서 제목 아래로 내린다.
- 거래처 목록 요약은 매우 좁은 모바일 폭에서만 2열 요약으로 전환한다.
...
카테고리처럼 열이 단순한 목록은 `840px 이하`에서 카드형 행으로 바꾼다.
...
입고 전표처럼 한 행의 업무 단위가 명확한 목록은 `640px 이하`에서 요약 카드형으로 압축한다.
...
입고 전표처럼 업무상 열 구분이 중요한 목록은 중간 폭에서 가로 스크롤을 허용한다.
...
```text
1600px: 데스크톱 사이드바, 햄버거 숨김, 2컬럼 겹침 없음
