...
재고 입출고 관리
입고 등록
거래처, 입고 사유, 입고 품목을 입력해 입고 전표를 생성합니다.
[전표 목록] [입고 저장]
```
...
- 전표 번호처럼 서버가 자동 발급하는 값은 입력 필드로 만들지 않는다.
- 거래처는 선택 목록으로 제공한다.
...
- 선택한 품목 영역은 `selected-part-panel`로 별도 강조한다.
- 선택한 품목, 수량, 품목 사유, 품목 추가 버튼이 한눈에 이어져야 한다.
- 품목 분류는 긴 배지보다 일반 텍스트에 가깝게 표시해 행 높이와 가독성을 안정적으로 유지한다.

## 입고 품목 목록 섹션

업무 전표에 추가된 입고 품목은 별도 목록 섹션에서 검토한다.
...
- 입고 품목이 없을 때는 `line-empty-state`를 보여준다.
- 품목 수는 제목 오른쪽의 작고 가벼운 텍스트로 둔다.
- 각 입고 품목은 독립된 항목처럼 보이되 카드 중첩이 과해지지 않게 한다.
...
- 보조 규칙은 꼭 필요한 경우에만 `muted-panel`에 3~4개 이하로 둔다.
- 입고 등록/검수/출고 등록의 오른쪽 패널은 workflow만 남기고, 별도 규칙 카드로 사이드바 스크롤을 만들지 않는다.
