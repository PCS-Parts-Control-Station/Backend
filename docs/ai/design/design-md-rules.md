...
페이지별 feature 문서나 페이지 설명 MD는 디자인 규칙의 소유 문서로 사용하지 않는다.

페이지 전용 MD에 남길 수 있는 내용:

- API, 필드, 상태, 권한, 저장 규칙 같은 도메인 계약
- 공통 화면 유형 중 이 페이지가 사용하는 조합
- 공통 규칙으로 표현할 수 없는 데이터 차이
- 예외가 필요한 업무상 이유와 적용 범위

페이지 전용 MD에 남기지 않는 내용:

- 색상, radius, 여백, 그림자, 버튼 크기
- 검색 카드, 목록, 오른쪽 패널, 모달의 공통 구조
- 공통 class의 상세 CSS 선언
- 다른 화면에서도 성립하는 반응형 규칙
- 특정 화면을 기준으로 만든 재사용 가능한 편집 카드나 상태 표현

페이지 문서에서 공통 디자인을 설명해야 할 때는 한 문장으로 소유 문서를 참조한다.

```text
화면 구성은 `docs/ai/design/management-page.md`를 따른다.
목록과 모바일 전환은 `data-table.md`와 `responsive-layout.md`를 따른다.
```

기존 페이지 MD를 수정할 때도 새 내용만 판단하지 않는다. 문서 전체를 확인해 공통 디자인 설명은 소유 문서로 이동하고, 페이지 문서에는 참조와 도메인 차이만 남긴다.
...
새 페이지 이름의 디자인 MD를 만드는 것은 기본 선택이 아니다. 기존 화면 유형 문서의 조합으로 설명할 수 없고, 다른 화면에서도 사용할 독립적인 화면 유형일 때만 새 `design/*.md`를 만든다.
...
디자인을 수정할 때 선언이나 규칙마다 아래 순서로 판정한다.

1. 기존 공통 MD/CSS에 이미 있는가?
   - 새로 작성하지 않고 기존 class와 규칙을 사용한다.
2. 기존 공통 규칙의 modifier로 표현할 수 있는가?
   - 역할 기반 modifier를 공통 문서와 공통 CSS에 추가한다.
3. 다른 화면에서도 같은 의미로 사용할 수 있는가?
   - 현재 사용 화면 수와 관계없이 공통으로 이동한다.
4. 데이터 열 수, 도메인 필드, 페이지 고유 상태 때문에 달라지는가?
   - 필요한 최소 선언만 페이지 MD/CSS에 남긴다.

페이지 전용으로 남긴다는 결론은 마지막 선택이다. 단순히 현재 한 화면에서만 사용한다는 이유는 페이지 전용 근거가 되지 않는다.

파일 줄 수만 줄이기 위해 서로 다른 의미의 컴포넌트를 억지로 합치지는 않는다. 최소화의 기준은 줄 수 자체가 아니라 공통 규칙 중복 제거와 페이지 예외의 축소다.
...
6. 페이지 전용 MD가 공통 디자인을 다시 설명하지 않는지 확인한다.
7. 페이지 CSS에 공통 컴포넌트 선언이 남아 있지 않은지 확인한다.
...
- 페이지 feature 문서에 공통 시각 규칙을 상세히 다시 정의하지 않는다.
- 현재 한 페이지에서만 쓴다는 이유만으로 재사용 가능한 디자인을 페이지 전용 MD에 둔다.
- 페이지 MD 수정 작업에서 기존 중복 규칙을 그대로 둔 채 새 규칙만 덧붙이지 않는다.
