-- 기준 스키마 재생성용 DDL입니다.
-- 기존 테이블을 DROP하므로 운영/실데이터 DB에서 직접 실행하지 마세요.
...
(
    ...
    CONSTRAINT `chk_member_owner_slot` CHECK (`role` = 'OWNER' and `owner_slot` is not null and `owner_slot` = 1 or
                                              `role` <> 'OWNER' and `owner_slot` is null)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
(
    ...
    CONSTRAINT `chk_stock_movement_before_after` CHECK (`before_quantity` >= 0 and `after_quantity` >= 0 and
                                                        (`movement_type` in ('INBOUND', 'OUTBOUND_CANCEL') and
                                                         `after_quantity` = `before_quantity` + `quantity` or
                                                         `movement_type` in ('OUTBOUND', 'INBOUND_CANCEL') and
