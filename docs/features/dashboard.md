...
- 우선 처리: 검수 대기, 판매 보류, 판매 불가, 오늘 불합격 검수처럼 사용자가 바로 처리해야 하는 항목
...
- 항목 유형 배지, 품목 분류, 부품명, 수량을 한 줄에 배치
- 행 전체 클릭으로 관련 화면 이동
- 검수 대기는 `partId`와 대기 보유 조건을 넘겨 `검수 관리`, 보류/불가는 `전표 통합 조회` 또는 재고 관련 화면으로 이동
...
- 최대 20개를 조회하고 화면에는 5개씩 표시한다.
- 이전/다음 버튼으로 페이지를 이동한다.
- 처리 유형 배지, 품목 요약, 전표 번호, 수량, 처리일 표시
...
- 전표 번호는 해당 관리번호와 연결된 입고 전표를 보조 조회한다.
...
- 우선 처리 응답은 프론트가 `type`을 재해석하지 않도록 실제 `route`와 필요한 `partState`를 함께 제공한다. 판매 보류와 판매 불가는 `route=part-units`와 각각 `STOCK_HOLD`, `STOCK_UNAVAILABLE`을 사용한다.
