...
부품 상세의 `검수하러가기`에서 `/w/{companyCode}/inspection?documentId={입고전표ID}&movementId={입고묶음ID}&unitId={관리번호ID}`로 진입하면 검수 화면은 해당 입고 전표, 부품 묶음, 관리번호를 자동 선택하고 3단계 검수 등록 영역을 표시한다.
