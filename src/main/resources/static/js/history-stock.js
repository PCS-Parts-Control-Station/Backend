(function () {
    ...
    const createBadgeCell = (label, type) => {
        const cell = document.createElement("span");
        cell.setAttribute("role", "cell");
        if (label) {
            cell.setAttribute("data-label", label);
        ...
        if (type === 'INBOUND') {
            ...
            badge.textContent = "입고";
        } else if (type === 'OUTBOUND') {
            badge.className = "badge badge-orange";
            badge.textContent = "출고";
        } else {
            badge.className = "badge badge-gray";
            badge.textContent = type || "알 수 없음";
        }
    ...
    const renderRows = (items) => {
        ...
        items.forEach((item) => {
            ...
            row.append(
                createBadgeCell("구분", item.documentType),
                createTextCell("전표번호", item.documentNumber, "code"),
                createTextCell("거래처", item.partnerName, "strong"),
    ...
    const loadHistory = async (page = 0, options = {}) => {
        ...
        const fetchPage = async (targetPage) => {
            ...
            const data = await window.PcsApi.getData(
                `/api/workspaces/${encodeURIComponent(companyCode)}/history/stock-documents?${params.toString()}`,
                {
                    authRedirect: true,
